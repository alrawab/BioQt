...
: io(io), z(NULL), buf(NULL), rewinded(0) {}
...
bool ZlibAdapter::open(const Url& url, IOAdapterMode m ) {
    assert(!isOpen());
    close();
...
qint64 ZlibAdapter::getUncompressedFileSizeInBytes(const Url &url) {
